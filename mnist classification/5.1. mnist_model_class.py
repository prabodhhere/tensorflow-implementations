...
class Model:
    def __init__(self, data, target, keep_prob):
        x_image = tf.reshape(data, [-1, 28, 28, 1], name='input')
        ...
        with tf.name_scope('cross_entropy'):
            self._cross_entropy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=target, logits=logits))
        with tf.name_scope('train'):
            self._train_step = tf.train.GradientDescentOptimizer(1e-4).minimize(self._cross_entropy)
        with tf.name_scope('correct_prediction'):
            self._correct_prediction = tf.equal(tf.argmax(logits, 1), tf.argmax(target, 1))
...
def main():
    ...
    with tf.Session() as sess:
        ...
        for i in range(1000):
            ...
            train_accuracy = sess.run(model.accuracy, {data: batch_train_xs, target: batch_train_ys, keep_prob: 0.5})
            if (step%100 == 0):
        ...
        for i in range(num_batches):
            ...
            test_accuracy += sess.run(model.accuracy, feed_dict={data: batch_test_xs, target: batch_test_ys, keep_prob: 1.0})
